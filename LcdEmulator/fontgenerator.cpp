...
const uint8_t mRawFontsData[] = { // 纵向排列的。
...
void FontGenerator::getActualCursor(QPoint cursor, QPointF *actualCursor) {
    ...
    actualCursor->setX((float) (mMarginWeight * mUnitWidth + mCharWidthOffset
                                * cursor.x()));
    actualCursor->setY((float) (mMarginWeight * mUnitHeight + mCharHeightOffset
                                * cursor.y()));
}
